...
const arquetipos: Record<'Curiosa' | 'Buscadora' | 'Estrategista' | 'Soberana', Omit<ArquetipoData, 'sinais'> & { sinaisFactory: (data: EspelhoData) => string[] }> = {
  ...
  Estrategista: {
    ...
    dorRaiz: 'Você funciona. Resolve. Entrega. Mas em algum ponto, você se trai — e isso aparece no corpo, no dinheiro ou nas emoções. O código existe. Falta virada.',
    ...
    primeiroPassoTexto: 'Responda o chamado. Transforme o que já funciona em ritual repetível — e sustente.',
  ...
  Soberana: {
    ...
    cicloRecomendado: 'Código 10 (Virada) + Cléo desbloqueada como prioridade',
...
export function buildPortalResetPayload(data: EspelhoData): PortalResetPayload {
  ...
  return {
    diagnostic: {
      ...
      sintomas,
      consequencia: `Tensão central em ${data.centralAxis.label} (${centralMicro.title.toLowerCase()}) — tensão ${data.centralAxis.tension.toFixed(0)}.`,
    ...
    report_screen: {
      ...
      acao_imediata: `Estabilizar ${data.bottom3[0].label} e reduzir tensão em ${data.centralAxis.label}.`,
